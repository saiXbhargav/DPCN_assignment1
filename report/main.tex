...
\begin{tikzpicture}[
    ...
    \node (m1_eval) [proc, below=0.6cm of m1, text width=4.6cm] {Model 1 Downstream Analysis\\$\bullet$ Erd\H{o}s--R\'enyi Null Model (50 runs)\\$\bullet$ Small-World Index ($\sigma = 1.8469$)\\$\bullet$ Louvain Modularity ($Q = 0.2840$)\\$\bullet$ Centrality Leaders (Hubs \& Bridges)};
...
\input{tables/tab1_global_metrics.tex}
...
\input{tables/tab2_student_centralities.tex}
...
\input{tables/tab3_community_profiles.tex}

\subsection{Community Structure and Thematic Personas}
Applying weighted Louvain modularity optimization on $G_S$ (\texttt{seed=42}) yields modularity $Q = 0.2840$, partitioning the cohort into \textbf{four primary opinion communities} ($95.40\%$ of students) plus \textbf{four singleton isolates} (Table~\ref{tab:community_profiles} and Figure~\ref{fig:community_question_analysis}a).
...
The empirical diameter ($\text{diam} = 4$) is twice the random baseline ($\text{diam} = 2$), reflecting \textbf{community modularity ($Q = 0.2840$)}: traversing between peripheral nodes across communities requires routing through intermediate bridge students (IDs \texttt{90}, \texttt{19}, \texttt{53}).

% --- Pre-queue Table 5 so it appears cleanly at the top of Page 7 ---
\input{tables/tab4_question_polarization.tex}
